    ...
    <div style="background: white;" @click="hideContextMenu">
        ...
        <div class="columns">
            <div class="column is-8 has-border-right">
                <nav class="nav has-shadow page-control product-images--page-control">
                    <div class="nav-left">
                        ...
                        <transition name="fade">
                            <div class="upload-dropdown product-images--upload-form" v-show="showUploadDropdown">
            ...
            <div class="column is-4">
                <label class="label">Primære Billede</label>
                <div class="media--text-align media--hover animated fadeIn">
                    <div class="selected-image" v-if="product.image">
                        <img 
                            style="width: auto; height: 150px; padding: 2px; box-shadow: 0 2px 2px 0 rgba(0, 0, 0, 0.3);"
                ...
                <div class="selected-images--container">
                    <div 
                        ...
                        @dblclick="makePrimary(image)"
                        v-for="image in productImages"
                    >
                        <span class="icon" @click="remove(image)">
                        ...
                        <img 
                            ...
                            alt="File Image"
                        >
                        <p class="product-images--name">{{ image.name }}</p>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            remove(media){
                this.$http
                    .post('/manufacturer/product/files', { file: media, product: this.data, _method: 'delete'} )
                    .then((response) => {
                        this.product.files = response.data;
                    });
            },
            makePrimary(media){
                this.$http
                    .patch('/manufacturer/product/files/primary', { file: media, product: this.data} )
                    .then((response) => {
        ...
        computed:{
            ...
            productImages(){
                ...
                return this.product.files.filter((file) => file.type == 'image');
            }
        },
        mounted() {
            let vm = this;
